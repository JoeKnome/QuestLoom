...
- [x] **Requirements tracked with threads** — Requirements are tracked with threads using **subtype** (`ThreadSubtype.REQUIRES` for entity-level, `ThreadSubtype.OBJECTIVE_REQUIRES` for quest objective dependency). Display labels are "Requires" and "Objective" from `getThreadSubtypeDisplayLabel`. Threads are visible on the Loom. Satisfaction uses a configurable allowed status set per target type (defaults: Item → [acquired], Insight → [known], Quest → [completed], Person → [alive]).
...
- [x] **Repository and types** — Add Path type, PathId, Path status enum, and PathRepository (CRUD, scoped by gameId). Threads can reference Path as source or target (extend EntityType to include Path). Cascade: deleting a Path removes its threads; deleting a Place removes threads that involved that Place (existing behavior).
- [x] **Loom behavior** — Paths appear as nodes; edges are Threads (Place–Path, Path–Place) and **direct Place–Place** threads (unimpeded). Traversability: direct Place–Place links are always traversable; for Paths, only **opened** Paths, or **restricted** Paths whose requirements are met, are traversable (per-path conditions apply). **Blocked** Paths are never traversable. Non-traversable Path connections are greyed out so they visually recede.
- [x] **UI** — Feature module for Paths: list, create, edit, delete. When editing a Path: name/description, **status** (restricted / opened / blocked), and requirement(s) for restricted Paths. UI shows connections for each Path via threads and supports configuring entity-level requirements; map marker creation supports Path as an eligible entity type.
- [x] All documentation pages are updated reflecting the latest state of the app.
- [x] All items left to do are documented for future action.
- [x] All affected code passes code standards, style, and lint.
...
- [ ] **Rules, discovery model, and toggles** — Define which entities/insights/threads/paths are visible only after certain conditions. Store visibility rules with **game data** and evaluate against **playthrough discovery state** and **playthrough-scoped statuses**. Use these rules for **optional spoiler-friendly filtering** in lists, the Loom, the Map view, and the Oracle: when filtering is **enabled**, undiscovered content is hidden or anonymized; when **disabled**, all entities are visible with only availability styling applied.
  - [ ] **Playthrough-level spoiler toggle** — Each playthrough has a boolean spoiler-filter setting (e.g. `spoilerFilteringEnabled`). When creating or editing a playthrough, the UI exposes this as a checkbox. Toggling this setting affects **only the visibility/filtering layer**; it never changes underlying discovery records or availability logic.
    - [ ] **Persistent discovery tracking** — Regardless of the checkbox value, discovery status for entities, places, and paths is always tracked internally for that playthrough. Turning spoiler filtering on later must immediately reflect the correct spoiler-safe view based on historical discovery records.
  - [ ] **Game-level default starting place** — At the game level, authors can configure a **default initial Place** (e.g. `defaultStartPlaceId`). When a new playthrough is created for that game, its **current position** (5.5) is initialized to this default Place; if none is set, the playthrough starts with no current position until the user selects one.
  - [ ] **Initial discovery at playthrough start** — When a playthrough begins, all entities are initially considered **undiscovered**. Immediately after setting the starting Place (from the game’s default or an explicit user choice), the system performs an **initial discovery evaluation** from that Place using the same rules described below: the starting Place is marked discovered, discovery cascades to its located entities and connected Paths, and any additional discovery rules are applied.
  - [ ] **Automated, place-driven discovery** — Managing spoiler and discovery state must be **as automated and low-friction as possible**, driven primarily by the user’s **current Place**:
    - [ ] When a Place is set as the **current place**, that Place is marked **discovered** for the playthrough.
    - [ ] All entities **located at** that Place (via LOCATION threads) are also marked **discovered**, regardless of whether they are currently available.
    - [ ] All **Paths connected** to that Place (via Place–Path connectivity threads) are marked **discovered** so their existence is known once the user has been there.
    - [ ] **Directly connected Places are not auto-discovered** purely because they are adjacent; a Place becomes discovered only once the user has actually **been there** (i.e. it has been set as current at least once). In short, **Place discovery drives discovery of everything else** rather than graph adjacency alone.
  - [ ] **Optional additional discovery requirements per entity** — Entities can have **optional additional discovery requirements** that gate when spoiler filtering clears for that entity. Discovery requirements reuse the generalized requirement model from 5.2 (status-based requirement threads) but are evaluated specifically for **spoiler visibility**, not for availability or traversability.
    - [ ] For such an entity to be considered fully "spoiler-cleared," the playthrough must both (1) have visited at least one of its location Places (so the entity is eligible for discovery) and (2) satisfy all of its configured discovery requirements. These are expected to be **exception cases**, used sparingly for narrative beats that must not be revealed immediately upon visiting the location.
  - [ ] **Separation from traversability and actionability** — Discovery and spoiler visibility **never** affect the underlying **traversability** logic (5.3–5.5) or **availability/actionability** logic (5.2, 5.6, 5.7). An entity can be **spoiler-hidden yet still actionable** according to contextual progression: the Oracle and Loom can highlight that there is an actionable step associated with an undiscovered objective, even if the entity itself is anonymized or hidden by spoiler filtering.
